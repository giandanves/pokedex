...
export function Pokemons(props) {
  ...
  return (
    <div className="shadow-shadow flex flex-wrap mx-auto max-w-fullscreen max-h-pokemoncontainer sm:max-h-full overflow-auto">
      {pokemons.length === 0 ? (
        <p>
        ...
        pokemons.map((pokemon, i) => {
          ...
          return (
            <div
              key={id}
              className={`shadow-shadow w-6/12 py-2 pr-4 sm:w-4/12 sm:pr-4 md:w-3/12 md:py-3 md:pr-8 lg:w-2/12`}
            >
              <li
                className={`shadow-shadow border border-${type[0]} rounded-md overflow-hidden`}
              >
                <div
                  className={`shadow-shadow w-full bg-${type[0]} bg-opacity-20 bg-gradient-to-br from-${type[0]} to-transparent rounded-br-2xl flex flex-col items-center p-3`}
                >
